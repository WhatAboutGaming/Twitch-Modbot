...
function setup() {
  ...
  restartModbotButton = createButton("RESTART MODBOT");
  restartModbotButton.position(5, 440);
  restartModbotButton.mousePressed(restartModbot);
  ...
  restartConnectionButton.position(5, 760);
...
function restartModbot() {
  console.log(new Date().toISOString() + " Someone pressed the RESTART MODBOT button!");
  socket.emit("restart_command", "restart_modbot");
}
...
function keyPressed() {
  if (key === "q" || key === "Q") {
    console.log(new Date().toISOString() + " Someone pressed Q on the keyboard to restart the modbot!");
    socket.emit("restart_command", "restart_modbot");
  }
  if (key === "p" || key === "P") {
