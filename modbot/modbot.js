  ...
  if (messageType == "chat" || messageType == "action") {
    ...
    */
    ...
    let singleMessageSpamBots = [
      ...
      //console.log("Test 70"),
      /((h+e+y)+|(h+e+l+o+)+|(h+i+)+|(h+o+w+d+y+)+)+\,*\s+((n+i+c+e+)+|(m+a+t+e+)+|(b+u+d+y*)+|(f+r+i*e+n+d*)+)+\,*\s*\,*((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)*\s+((m+e+t+i+n+g+)+|(s+t+r+[e*a*]*m+i*n*g*e*r*s*)+)+\s+(p+r+e+t+y+)+\s+(c+o+l+)+\,*\s+\,*(t+h+a+t+[\'\’]*\s*[IiSs]*)+\s+(w+h+y+)+\s*(\w*)*\s*(f+o+l+o+w+\w*)+\s*((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)*/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")), // Howdy mate, You stream pretty cool that’s why I followed you. I will like to make you a friend and be a fan, if you don’t mind Kindly chat me on Discord, my Discord username is markk_2 (PART 2)
      //console.log("Test 71"),
