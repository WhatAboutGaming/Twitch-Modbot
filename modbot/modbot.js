...
async function onMessageHandler(target, tags, message, self) {
  ...
  */
  ...
  if (messageType == "chat" || messageType == "action") {
    ...
    //let checkTwitchViewBotSoftwareBot = /(t+w+\w*t+c+h+)+\s+(v+i+e+w+\w*\s*b+[o0]+t+\w*)+\s+(s+o+f+t+w+a+r+e+\w*\W*)+\s*(d+o+)+\s+(a+n+y+)+\s*(o+n+l+i+n+e+)*\s+(\w*)+\s+(a+n+y+)+\s+(s+t+r+e+a+m+\w*\W*\w*)+\s+(\W*\d*\W*)+\W*\s*((d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+\s+\W*\s+(\w+\W*\d+))*/ig.test(message.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, ""));
    ...
    */
    ...
    //let checkDiscordStreamersCommunityBot = /(h+e+y+\W*)+\s+(n+i+c+e+)+\s+(s+t+r+e+a+m+\w*\W*\w*\W*)+\s+(y+\w*)+\s+(s+h+\w*)+\s+(f+o+r+)+\s+(s+u+r+e+)+\s+(j+o+i+n+)+\s+(\w*)+\s+(s+t+r+e+a+m\w*\W*\w*)+\s+(c+o+m+u+n+i+t+y+)+\s+(\w*)+\s+(j+u+s+t+)+\s+(f+o+u+n+d+)+\s+(\w*)+\s+(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+\s+(y+e+s+t+e+r+d+a+y+)+\s+([^\n\s]*)+\s+(c+h+e+c+k+)+\s+(i+t+)+\s+(o+u+t+)+/ig.test(message.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig));
    ...
    //  /(t+w+\w*t+c+h+)+\s+(v+i+e+w+\w*\s*b+[o0]+t+\w*)+\s+(s+o+f+t+w+a+r+e+\w*\W*)+\s*(d+o+)+\s+(a+n+y+)+\s*(o+n+l+i+n+e+)*\s+(\w*)+\s+(a+n+y+)+\s+(s+t+r+e+a+m+\w*\W*\w*)+\s+(\W*\d*\W*)+\W*\s*((d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+\s+\W*\s+(\w+\W*\d+))*/ig.test(message.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")),
    //  /(h+e+y+\W*)+\s+(n+i+c+e+)+\s+(s+t+r+e+a+m+\w*\W*\w*\W*)+\s+(y+\w*)+\s+(s+h+\w*)+\s+(f+o+r+)+\s+(s+u+r+e+)+\s+(j+o+i+n+)+\s+(\w*)+\s+(s+t+r+e+a+m\w*\W*\w*)+\s+(c+o+m+u+n+i+t+y+)+\s+(\w*)+\s+(j+u+s+t+)+\s+(f+o+u+n+d+)+\s+(\w*)+\s+(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+\s+(y+e+s+t+e+r+d+a+y+)+\s+([^\n\s]*)+\s+(c+h+e+c+k+)+\s+(i+t+)+\s+(o+u+t+)+/ig.test(message.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig)),
    ...
    let singleMessageSpamBots = [
      ...
      //console.log("Test 65"),
      /((\w+[\'\’]+d+)+\s+)*(l+[ou]+v+e*)+(\s+t+o+)*\s+((c+h+a+t)+|(a+d+)+|(t+a+l+k+)+)+\s+(w+i+t+h*)+\s+((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)+(\s*m+o+r+e+)*\s+(a*b+o+u+t*)+(\s+i+t+)*(\s*\w+)\s+(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")), // Hi mate nice streaming I noticed something on your channel that I think needs to be amended. I’d love to chat with you more about it on Discord. It would be a great opportunity to discuss the issue in more detail and to come up with a solution together. Would you be willing to chat with me on Discord? My username is Diablous#8216 (PART 2)
      //console.log("Test 66"),
      ...
      //console.log("Test 68"),
      /(((w+o+u+l+d+)+|([\'\’]d)+)+)+\s+((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)+(\s+(b+e+)+)*\s+(w+i+l+i*n*g*)+(\s+t+o+)*\s+((c+h+a+t)+|(a+d+)+|(t+a+l+k+)+)+\s+(w+i+t+h*)+\s+(m+e+)+(\s*(\s*\w+)\s*(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+)*/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")), // Hi mate nice streaming I noticed something on your channel that I think needs to be amended. I’d love to chat with you more about it on Discord. It would be a great opportunity to discuss the issue in more detail and to come up with a solution together. Would you be willing to chat with me on Discord? My username is Diablous#8216 (PART 5)
      //console.log("Test 69"),
      ...
      //console.log("Test 72"),
      /(i+f+)+\s*((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)*\s+(d+o+n+[\'\’]*t+)+\s+(m+i+n+d+)+\s*\,*\s*(k+i+n+d+l+y+)*\s+((c+h+a+t)+|(a+d+)+|(t+a+l+k+)+)+\s*(m+e+)*\s+(o+n+)+\s+(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")), // Howdy mate, You stream pretty cool that’s why I followed you. I will like to make you a friend and be a fan, if you don’t mind Kindly chat me on Discord, my Discord username is markk_2 (PART 4)
      //console.log("Test 73"),
      ...
      //console.log("Test 77"),
      ///(c+o+u+l+d+)*\s*((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)*\s*((c+h+a+t)+|(a+d+)+|(t+a+l+k+)+)+\s*(m+e+)*\s+(o+n+)+\s+(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")), // Hey, mate, I really enjoy your streams,you stream pretty cool and I love it . I'd love to be part of your fan community. Could you add me on Discord? My username 👉👉👉ayofe_1 (PART 5)
      //console.log("Test 78"),
      /(m+y+)+\s*(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)*\s*(u+s+e+r+)+\s*(n+a+m+e+)+/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")), // Hey, mate, I really enjoy your streams,you stream pretty cool and I love it . I'd love to be part of your fan community. Could you add me on Discord? My username 👉👉👉ayofe_1 (PART 6) (Do Not Use, Could cause false positives)
      //console.log("Test 79"),
      ...
      /((l+e+t+[\'\’]*\s*[UuSs]*)*\s*(\w*)\s*(c+o+n+[te]*a*c+t+\w*)+\s*(\w*)\s*(\w*)\s*(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+)+/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")), // [Sat 2024-11-09T15:38:46Z]  <wumight4> Hey streamer 👋,I’ve been really enjoying your content, which is why I’m part of your squad. I noticed you’re facing some challenges with growing your channel and leveling up. I’d love to share ideas and help elevate your stream. Let’s connect on Discord: Femight7. (Let's connect on Discord) / (Let's stay connected on Di  s c o r d)
    ...
    let multiMessageSpamBotTypeC = [
      ...
      /((g+i+v+e*)+\s*(m+e+)*\s*(\w*)*\s+(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+)+/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, ""))
    ];
    let multiMessageSpamBotTypeD = [
      ...
      /(s+e+n+[dt]+)+\s+(m+e+)+\s+((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)+\s+(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")), // Send me your discord
      /(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+\s+(s+e+n+[dt]+)+\s+(m+e+)+\s+((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)+\s+(r+e+q+u+e+s+t+)+\s+(s+o+)+\s*(\w*)\s*(c+a+n+)+\s+(s+h+a+r+e+)+\s+(m+y+)+\s+(w+o+r+k+)+/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")), // alisadaniel its ,my discord sent me your request so i can share my work <3
      /(t+y+p+e*)+\s+((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)+\s+(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")) // type your discord ?
    ];
    let multiMessageSpamBotTypeE = [
      ...
      /(h+a+v+e+)+\s*(\w*)*\s*(i+m+p+o+r+t+a+n+t+)+\s+(t+a+l+k+)+\s+(\w*b+o+u+t+)+\s+((y+o+)+|(u+\s*r*)+\s*\w*\s*[\'\’]*\s*\w*)+\s+(c+h+a+n+e+l+)+\s*(\w*)\s*(c+o+n+[te]*a*c+t+\w*)+\s+(m+e+)(\s*(\s*\w+)\s*(d+\s*i+\s*s+\s*c+\s*o+\s*r*\s*d*)+)*/ig.test(replaceCyrillicsWithLatin.normalize("NFD").replace(/[\u007E-\uFFFF]+/ig, "")) // I have an important talk about your channel kindly contact me on discord. Pearl0307
    ];
